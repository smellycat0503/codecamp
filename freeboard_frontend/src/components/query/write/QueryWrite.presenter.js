    ...
    return (
        ...
        <UserInfo>
            <Name>
                ...
                <NameInput type = "text" placeholder="이름을 적어주세요." name = "writer"  onChange = {props.onChangeInput}></NameInput>       
                <ErrorMessage1>{error}</ErrorMessage1>
            </Name>
            <Password>
                <PasswordTitle>비밀번호</PasswordTitle>
                <PasswordInput type = "password" placeholder="비밀번호를 입력해주세요." name="password" onChange = {props.onChangeInput}></PasswordInput>
                <ErrorMessage1>{error}</ErrorMessage1>
            </Password>
        </UserInfo>
        ...
        <SubTitleWrapper>
            ...
            <SubTitleInput type = "text" placeholder="제목을 작성해주세요." name = "title" onChange = {props.onChangeInput}></SubTitleInput>
            <ErrorMessage1>{error}</ErrorMessage1>
        </SubTitleWrapper>

        <ContentWrapper>
            <ContentTitle>내용</ContentTitle>
            <ContentInput type = "text" placeholder= "내용을 작성해주세요." name = "contents" onChange = {props.onChangeInput}></ContentInput>
            <ErrorMessage1>{error}</ErrorMessage1>
        </ContentWrapper>
        ...
        <RegistWrapper>
            <RegistButton onClick = {ErrorMessage} onClick={props.onClickPost} isTrue = {props.isTrue}>등록하기</RegistButton>
        </RegistWrapper>
